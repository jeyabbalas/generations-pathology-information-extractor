...
`},{role:"user",content:n.prompt}],temperature:this.temperature,seed:this.seed+s}),a=i.choices[0].message.content,r=a.match(o),r)return JSON.parse(r[1]);s++}return console.error("Failed to extract valid JSON after 3 attempts, object with all missing values."),n.default}}function J(t){let e=t?document.getElementById(t):document.createElement("div");e.innerHTML=`
<!-- Header -->
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex md:mt-0 md:ml-4 shrink-0">
            <a title="Source code" href="https://github.com/jeyabbalas/generations-pathology-information-extractor">
                <img src="${S}" class="h-10 w-10 sm:h-12 sm:w-12 fill-current" alt="github logo" />
            </a>
        </div>
    </div>
</div>
...
`}J("app");let x=null;const _=document.getElementById("baseURL"),M=document.getElementById("apiKey"),q=document.getElementById("forget-api-key"),k=document.getElementById("submit-api-key"),g=document.getElementById("llmModel"),u=document.getElementById("api-key-message-container");q.addEventListener("click",function(){for(M.value="";u.firstChild;)u.removeChild(u.firstChild);T(),L.deleteKey()});g.addEventListener("change",function(){x!==null&&x.setModel(this.value)});k.addEventListener("click",async function(){const t=document.getElementById("apiKey"),e=_.value,n=t.value,o=await L.validateApiKey(e,n);for(;u.firstChild;)u.removeChild(u.firstChild);if(o){L.setKey(n),x=await E.instantiate({baseURL:e,apiKey:n});const s=x.getModelsList();g.innerHTML="",g.disabled=!1,s.forEach(function(a,r){const l=document.createElement("option");l.value=a,l.text=a,g.appendChild(l),r===0&&(g.value=a,x.setModel(g.value))});const i=document.createElement("div");i.classList.add("bg-green-50","border","border-green-300","text-green-800","px-4","py-3","rounded","relative","mt-4"),i.innerHTML='<strong class="font-bold">Success!</strong> API key validation was successful.',u.appendChild(i)}else{T();const s=document.createElement("div");s.classList.add("bg-red-50","border","border-red-300","text-red-800","px-4","py-3","rounded","relative","mt-4"),s.innerHTML='<strong class="font-bold">Error!</strong> The input LLM API URL or API key was invalid.',u.appendChild(s)}});function T(){g.innerHTML='<option value="" disabled selected>Set URL/API key above to see models list</option>'}(async()=>{const t=L.getKey();t&&(M.value=t,k.click())})();const w=document.getElementById("info-extraction"),O=document.getElementById("standardization"),v=[];async function j(t){v.length=0;let e=[];const n=document.getElementById("file-upload-icon"),o=document.getElementById("progress-ring"),s=o.querySelector(".progress-ring-text"),i=document.getElementById("files-uploaded-message");n.classList.add("hidden"),o.classList.remove("hidden"),i.textContent="",[...t].forEach((a,r)=>{const l=new FileReader;let d=new Promise((m,c)=>{l.onload=function(p){v.push({name:a.name,data:p.target.result}),s.textContent=`${r+1}/${t.length}`,m()},l.onerror=c,l.readAsText(a)});e.push(d)}),await Promise.all(e),o.classList.add("hidden"),n.classList.remove("hidden"),i.textContent=`${v.length} files uploaded successfully.`}const y=document.getElementById("file-drop-area");function Y(t){t.preventDefault(),t.stopPropagation()}["dragenter","dragover","dragleave","drop"].forEach(t=>{y.addEventListener(t,Y,!1)});function W(t){y.classList.add("border-green-900","border-2")}function X(t){y.classList.remove("border-green-900","border-2")}["dragenter","dragover"].forEach(t=>{y.addEventListener(t,W,!1)});["dragleave","drop"].forEach(t=>{y.addEventListener(t,X,!1)});function Z(t){const n=t.dataTransfer.files;j(n)}y.addEventListener("drop",Z,!1);const B=document.getElementById("file-upload");B.addEventListener("change",t=>{const e=t.target.files;j(e)});const G=document.getElementById("reset-btn");G.addEventListener("click",t=>{v.length=0,B.value="",w.innerHTML="",O.innerHTML="",document.getElementById("files-uploaded-message").textContent=""});const f=document.getElementById("submit-btn"),h=[];function Q(t,e,n){n.innerHTML="";const o=document.createElement("div");o.className="relative overflow-x-auto shadow-md sm:rounded-lg my-4 w-full";const s=document.createElement("table");s.className="w-full text-sm text-left text-gray-500";const i=document.createElement("thead");i.className="text-xs text-white uppercase bg-green-900";const a=document.createElement("tr");t.forEach(l=>{const d=document.createElement("th");d.scope="col",d.className="px-6 py-3",d.textContent=l,a.appendChild(d)}),i.appendChild(a),s.appendChild(i);const r=document.createElement("tbody");e.forEach(l=>{const d=document.createElement("tr");d.className="bg-white border-b hover:bg-green-50",t.forEach(m=>{const c=document.createElement("td");c.className="px-4 py-2",c.textContent=l[m]??"-",d.appendChild(c)}),r.appendChild(d)}),s.appendChild(r),o.appendChild(s),n.appendChild(o)}f.addEventListener("click",async t=>{const e=f.innerHTML;if(f.innerHTML=`
                ...
                Extracting data...
            `,f.disabled=!0,w.innerHTML="",O.innerHTML="",v.length===0){alert("Please upload at least one file.");return}h.length=0;let n=[];const o=m=>new Promise(c=>setTimeout(c,m)),s=3e3,i=async(m,c)=>{await o(c);let p=await x.extractInformation(m.data);const b=m.name.split(".");p={id:b.length>1?b.slice(0,-1).join("."):m.name,...p},h.push(p)};v.forEach((m,c)=>{let p=new Promise(async(b,R)=>{try{await i(m,c*s),b()}catch(K){R(K)}});n.push(p)}),await Promise.all(n);const a=Object.keys(h[0]);h.sort((m,c)=>m.id.localeCompare(c.id));const r=document.createElement("div");r.className="flex justify-center w-full",Q(a,h,r),w.appendChild(r);const l=document.createElement("div");l.className="flex justify-center w-full my-2";const d=document.createElement("button");d.className="inline-flex justify-center rounded-md border border-transparent bg-green-800 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2",d.textContent="Download as JSON",d.addEventListener("click",m=>{const c="data:text/json;charset=utf-8,"+encodeURIComponent(JSON.stringify(h,null,2)),p=document.createElement("a");p.setAttribute("href",c),p.setAttribute("download","extracted_information.json"),p.click()}),l.appendChild(d),w.appendChild(l),f.innerHTML=e,f.disabled=!1});
